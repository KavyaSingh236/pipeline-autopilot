...
async def _seed_data() -> None:
    """Populate bronze/silver/gold with synthetic Olist-style rows (idempotent)."""
    assert _pool is not None
    async with _pool.acquire() as conn:
        count = await conn.fetchval("SELECT count(*) FROM bronze.raw_orders")
        ...
        for i in range(1000):
            ...
            orders.append((oid, cid, random.choice(_STATUSES), ts, round(random.uniform(15, 1200), 2)))

        await conn.executemany(
            "INSERT INTO bronze.raw_customers VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING", customers)
        await conn.executemany(
            "INSERT INTO bronze.raw_products VALUES ($1,$2,$3) ON CONFLICT DO NOTHING", products)
        await conn.executemany(
